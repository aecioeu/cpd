...
function ($) {
    ...
    */
     ...
     */
     ...
     var bodyConfig = {"mode": "light", "width": "fluid", "menuPosition": "fixed", "sidebar": { "color": "light", "size": "default", "showuser": false}, "topbar": {"color": "light"}, "showRightSidebarOnPageLoad": true}
...
let taskInitiCount = window.localStorage.getItem('task_count');
//let taskInitiMyCount = window.localStorage.getItem('my_task_count');

if(taskInitiCount){
//console.log(taskInitiCount)
    taskCount(JSON.parse(taskInitiCount))
}
/*if(taskInitiMyCount){
    //console.log(taskInitiCount)
    taskMyCount(JSON.parse(taskInitiCount))
}*/
    /*
function taskMyCount(myResponse){

    window.localStorage.setItem('my_task_count', JSON.stringify(myResponse));   
    console.log("Minhas tarefas",  myResponse)
    $(".mytasks").text('0')
    $(".mytasks").text(myResponse[0].count)

}*/

function taskCount(response) {
    
    window.localStorage.setItem('task_count', JSON.stringify(response));


   // console.log(response.myTasks, response.allTasks)

    
        $('.count_task').text('0')
        response.forEach(function(item) {
        $(`.${item.status}`).each(function(index, element) {
       ...
       /* $(".mytasks").text('0')
        $(".mytasks").text(`${response.myTasks[0].count}`)*/
...
function togglePrinterStatus(isOnline) {
    // Seleciona o elemento com a classe 'printer_status'
    const printerStatusElement = document.querySelector('.printer_status');

    if (printerStatusElement) {
      if (isOnline) {
        // Se a condição for verdadeira, altera de 'bg-danger' para 'bg-success'
        printerStatusElement.classList.remove('bg-danger');
        printerStatusElement.classList.add('bg-success');
      } else {
        // Se a condição for falsa, altera de 'bg-success' para 'bg-danger'
        printerStatusElement.classList.remove('bg-success');
        printerStatusElement.classList.add('bg-danger');
      }
    } else {
      console.error("Elemento '.printer_status' não encontrado.");
    }
  }

  function autoPrint(id, solicitante, location, descricao, unit, total){

    console.log("Imprimindo", qz)
    
      var config = qz.configs.create("ETIQUETA" ,{
      size: {width: 150, height: 30}, units: 'mm',
        ...
        data: `<html>
          <style>
              body {
                  ...
                  align-items: center;
                  width: 100mm; /* Define a largura da etiqueta */
              ...
              .left {
                  width: 120px; /* Define a largura do lado esquerdo */
                  display: flex;
              ...
              .right {
                  width: 300px; /* Define a largura do lado direito */
                  display: flex;
              ...
              h1 {
                  margin: 0;
                  padding-bottom:2px;
                  font-size: 9pt; /* Ajuste o tamanho da fonte conforme necessário */
              }
          ...
          padding: 1px;}
              ...
              .descricao{
                 font-size: 10pt;
      font-weight: 400;
      word-wrap: break-word; /* Quebra a linha quando necessário */
      overflow-wrap: break-word; /* Quebra a linha quando necessário */
              }
          </style>
          ...
          <div class="label">
            <div></div>
              <div class="left">
                  <h2><small>NÚMERO DA O.S</small>#${id}</h2>
                  <h3>VOLUME: ${unit}/${total}</h3>
              </div>
              <div class="right">
                  <h1><small>SOLICITANTE</small>${solicitante}</h1>
                  <h1 style="font-size: 8px;"><small style="margin-bottom: 1px;">SETOR/LOCAL</small>${location}</h1>
                  <h1 style="font-size: 8pt;"><small>PROBLEMA RELATADO</small> <span class="descricao">Computador travando, vamos ver como é tratado um texto grande, que excede o tamanho da etiqueta.</span></h1>
              </div>
          </div>
          </body>
          </html>
`
  ...
  function checkPrintEtiquetas(id, solicitante, location, descricao, message, value){
    ...
    bootbox.prompt({
        ...
        callback: function (result) {
            if(result){
             //console.log("iajisdisajd" + result)
            
            // autoPrint(id, solicitante, location, descricao, unit, total)

             for (let i = 0; i < result; i++) {
                var unit = i +1

                console.log(i, `IMPRIMINDO ${unit} de ${result}`)
                autoPrint(id, solicitante, location, descricao, unit, result)
